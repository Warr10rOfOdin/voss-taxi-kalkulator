...
### Example 3: Multi-Tenant React App with Config Sync
...
export function watchCustomerConfig(customerId: string, callback: (config: any) => void) {
  onValue(ref(db, `tenantRegistry/${customerId}/config`), (snap) => {
    callback(snap.val());
  });
}
...
function App() {
  ...
  const customerId = resolveCustomerFromDomain();

  useEffect(() => {
    watchCustomerConfig(customerId, setConfig);
  }, [customerId]);

  // Apply theme CSS variables from CTRL BOARD
  useEffect(() => {
    if (config?.theme) {
  ...
  return (
    <div>
      ...
      {config?.features?.showDashboard && <Dashboard />}
      <MainContent />
    </div>
  );
}
...
**See Also:** [INDEX.md](./INDEX.md) | [WHAT_IS_CTRL_BOARD.md](./WHAT_IS_CTRL_BOARD.md) | [GETTING_STARTED.md](./GETTING_STARTED.md) | [APP_PROFILE_TEMPLATE.md](./APP_PROFILE_TEMPLATE.md)
